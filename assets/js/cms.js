$('#add-row').DataTable({
	columnDefs: [ { orderable: false, targets: 0, width: '1%' } ],
	order: [ [ 1, 'asc' ] ]
});
	...
	$.get(
		...
		function(ex) {
			...
			$('#dz-SJ').dropzone({
				...
				dictRemoveFile: 'Hapus file',
				init: function() {
					if (ex.data.DO_DATE != null) {
						var myDropzone = this;

						var mockFile = {
							name: ex.data.DO_FILE,
							size: 12345,
							accepted: true
						};
						...
						myDropzone.files.push(mockFile);
						myDropzone.emit('addedfile', mockFile);
						myDropzone.emit('thumbnail', mockFile, baseUrl + '/assets/doc/' + ex.data.DO_FILE);
						myDropzone.emit('complete', mockFile);

						myDropzone.on('thumbnail', function(_file) {
							console.log('created');
						});
					}
				},
			...
			$('#dz-invoice').dropzone({
				...
				dictRemoveFile: 'Hapus file',
				init: function() {
					if (ex.data.INVOICE_FILE != null) {
						var myDropzone = this;

						var mockFile = {
							name: ex.data.INVOICE_FILE,
							size: 12345,
							accepted: true
						};
						...
						myDropzone.files.push(mockFile);
						myDropzone.emit('addedfile', mockFile);
						myDropzone.emit('thumbnail', mockFile, baseUrl + '/assets/doc/' + ex.data.INVOICE_FILE);
						myDropzone.emit('complete', mockFile);
		...
		$.post(
			...
			function(ex) {
				if (ex.code === 200) {
					swal({
						title: 'Update Data',
						text: 'Proses update SO berhasil',
						icon: 'success',
						buttons: {
							confirm: {
								text: 'Kembali',
					...
					swal({
						title: 'Update Data',
						text: 'Proses Update SO gagal, silahkan coba kembali',
						icon: 'error',
						buttons: {
							confirm: {
								text: 'Kembali',
